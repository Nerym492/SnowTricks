...
const profilePictureFile = document.getElementById(
  "registration_form_profilePictureFile",
);
  ...
  for (let mutation of mutations) {
    if (mutation.type === "childList" && mutation.addedNodes.length > 0) {
      mutation.addedNodes.forEach((addedNode) => {
        if (
          addedNode.classList.contains("invalid-field-text") &&
          addedNode.parentElement.id === "profile-picture-group"
        ) {
...
document
  .getElementById("registration_form_profilePictureFile")
...
document
  .getElementById("delete-profile-picture")
  .addEventListener("click", () => {
    resetFileInput();
  });

registerValidator
  .addField("#" + profilePictureFileId, [addFileValidationRules()])
...
formErrorsObserver.observe(
  document.getElementById("profile-picture-group"),
  observerOptions,
);
