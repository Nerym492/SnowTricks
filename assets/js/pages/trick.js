...
const imagePlaceholder =
  '<img class="image-trick-details img-form" src="' +
  imagePlaceholderSrc +
  '" alt="">';
...
let previewObserver = new MutationObserver(function (mutationsList) {
  for (let mutation of mutationsList) {
    if (
      ...
      if (borderName === "top") {
        let allInputsIsHeader = document.querySelectorAll(
          ".trick-form-isheader",
        );
        allInputsIsHeader.forEach((input) => {
          input.value = "0";
        });
        let inputIsHeader = addedElement
          .closest(".trick-image-item")
          .querySelector(".trick-form-isheader");
        inputIsHeader.value = "1";
      }
      animateBorder(
        addedElement,
        animationsBorders[borderName].fadeIn,
        "fadeIn",
      ...
      mutation.addedNodes[0]
        .closest(".trick-image-item")
        .querySelector(".trick-form-isheader").value = "1";
    } else if (
      mutation.type === "childList" &&
      mutation.addedNodes.length > 0 &&
      mutation.target === mutation.addedNodes[0].parentElement
    ) {
      let borders = mutation.addedNodes[0]
        .closest(".trick-image-preview")
        .querySelectorAll(".border-isheader");
      // if the image is defined as favorite, change the header
      if (borders.length > 0) {
        document.getElementById("image-header-details").src =
...
let imageHeaderObserver = new MutationObserver(function (mutations) {
  ...
  if (
    ...
    lastMutation.attributeName === "src"
  ) {
...
function addDeleteImageListener(deleteButton) {
  deleteButton.addEventListener("click", function () {
    ...
    animateElement(trickImgItem, "img-item-zoom-out", () => {
      trickValidator.removeField(
        "#" + trickImgItem.querySelector(".image-file-actions input").id,
      );
      trickImgItem.remove();
...
function addInputFileValidation(input) {
  trickValidator.addField("#" + input.id, [
    ...
    },
    {
      ...
      errorMessage:
        "The file must be an image (png, jpeg, jpg, webp). Maximum size 600kb",
    },
  ]);
}
...
function displayImagePreview(files, preview) {
  if (files && files.length > 0) {
    ...
    let borderIsHeader = preview
      .closest(".medias-container")
      .querySelectorAll(".border-isheader");
    const file = files[0];
    ...
    reader.onloadend = function () {
      ...
      imageLink.replaceChild(
        img,
        preview.querySelector(".image-trick-details"),
      );
      // No header set
...
function addLinkFavoriteImage(preview) {
  ...
  previewObserver.observe(
    preview.querySelector(".favorite-image"),
    observerOptions,
  );
  return link;
}
...
function reassignIsHeaderImage(tempDiv) {
  ...
  if (otherFavoriteImages.length > 0) {
    ...
    let firstValidPreview = otherFavoriteImages[0].closest(
      ".trick-image-preview",
    );
    borders.forEach((border) => {
...
function moveFavoriteImgBorders(preview) {
  ...
  animateBorder(
    borderRight,
    animationsBorders.right.fadeOut,
    "fadeOut",
    preview,
  );
...
function addImageForm(imagePlaceholder) {
  let newForm = createItemFromPrototype(
    imagePlaceholder,
    imagesCollection,
    addImageFormButton,
  );
  ...
  let deleteButtonHtml =
    '<a role="button" class="delete-image-btn" id="delete-img-btn-' +
  ...
  addDeleteImageListener(
    document.getElementById("delete-img-btn-" + imagesCollection.index),
  );
...
function addVideoLinkValidation(link) {
  trickValidator.addField("#" + link.id, [
    {
      rule: "required",
    ...
    },
    {
      ...
      errorMessage:
        "The link is not valid.<br>https://www.youtube.com/embed/code",
    },
  ]);
}
...
function addVideoLinkKeyup(link) {
  link.addEventListener("keyup", () => {
    let preview = link
      .closest(".trick-video-item")
      .querySelector(".trick-video-preview");
    if (link.classList.contains("just-validate-success-field")) {
      preview.innerHTML =
        '<iframe class="trick-video" src="' +
        link.value +
...
function addDeleteVideoListener(button) {
  button.addEventListener("click", () => {
    ...
    animateElement(trickVideoItem, "img-item-zoom-out", () => {
      trickValidator.removeField(
        "#" + trickVideoItem.querySelector("input").id,
      );
...
imageHeaderObserver.observe(headerImage, {
  attributes: true,
  attributeFilter: ["src"],
});
addMobileMenuEvent();
...
if (formError) {
  ...
  let formErrorPosition =
    formError.getBoundingClientRect().top + window.scrollY;
